...
document.getElementById('btn-reset').addEventListener('click', () => {
    if (confirm('Reset all metrics and restart the timer?')) socket.emit('admin_reset_game');
});

socket.on('scenario_changed', (id) => {
    const el = document.getElementById('admin-current-scenario');
    if (el) el.textContent = id === 1 ? 'Scenario 1 — Legacy Grid' : 'Scenario 2 — Smart Grid';
});
...
btnStartQuiz.addEventListener('click', () => {
    ...
    quizLiveCount.textContent = '0 answered';
    showToast('📝 Quiz started!', 'info');
});
...
socket.on('simulation_ended', (payload) => {
    ...
    if (resultsScenario) resultsScenario.textContent = scenario;
    ...
    if (ctx) {
        ...
        window.__resultsChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['Outages', 'Help Calls', 'Resolved', 'DR Accepted', 'Grid Strain (MW)'],
                datasets: [{
    ...
    if (snapshots && (snapshots[1] || snapshots[2]) && compSection) {
        ...
        function renderSnap(el, snap) {
            if (!el || !snap) return;
            el.innerHTML = `
                <p class="text-red-400">Outages: <span class="font-black">${snap.outages || 0}</span></p>
                <p class="text-yellow-400">Help Calls: <span class="font-black">${snap.callsMade || 0}</span></p>
                <p class="text-green-400">Resolved: <span class="font-black">${snap.issuesResolved || 0}</span></p>
                <p class="text-blue-400">DR Accepted: <span class="font-black">${snap.drAccepted || 0}</span></p>
            `;
        }
    ...
    if (timelineList && timeline && timeline.length > 0) {
        ...
        timelineList.innerHTML = timeline.map(ev => {
            const cfg = typeConfig[ev.type] || { icon: '•', color: 'text-gray-400' };
        ...
        timelineList.innerHTML = '<p class="text-gray-600 text-sm text-center py-4">No events recorded this session.</p>';
    }
});
